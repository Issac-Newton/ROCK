...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    "ROCK_RTENV_NODE_V22180_INSTALL_CMD": lambda: os.getenv(
        ...
        "[ -f node.tar.xz ] && rm node.tar.xz; [ -d node ] && rm -rf node; wget -q -O node.tar.xz --tries=10 --waitretry=2 https://xrl-sandbox-bucket.oss-cn-hangzhou.aliyuncs.com/wanqi-test/node-v22.18.0-linux-x64.tar.xz && tar -xf node.tar.xz && mv node-v22.18.0-linux-x64 runtime-env",
    ),
